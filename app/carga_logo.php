...
if ($_FILES["fileToUpload"]["size"] > ((1024*1024)*20)) {
    ...
    if (file_exists($target_file) or (strtoupper($imageFileType) != ("PNG") or strtoupper($imageFileType) != ("JPG")) ){
        echo "El Archivo que intenta cargar, ya existen en el Sistema, se intenta subir un duplicado <p>";
        echo "o el archivo no es valido; solo se pueden subir arvhivos PNG y JPG. </p>";
        $controller->Empresas();
    }else{
        if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
            echo "El Archivo: ". basename( $_FILES["fileToUpload"]["name"]). " se ha cargado.<p>";
            $tipo = 'ok';
            $cambio=$controller->cargaLogo($ide, $fileName);
        } else {
